# 🧠 Brain Dump

A small place for notes and ideas.

[Open Brain Dump](https://afnannuzulasugihartono.github.io/brain-dump/) · [New note](https://github.com/afnannuzulasugihartono/brain-dump/issues/new?template=idea.yml) · [Issues](https://github.com/afnannuzulasugihartono/brain-dump/issues) · [AI access](docs/ai-access.md)

---

<!-- TIMELINE:START -->
## Recent ideas

_11 ideas · 10 open_

### 24 Sep 2026 · [#18 Indonesia Corruption Watch website](https://github.com/afnannuzulasugihartono/brain-dump/issues/18)

Information about corruption cases is often scattered across news reports, court records, government releases, and watchdog publications. A structured, searchable, source-linked website could make public-interest information easier to follow and compare over time.

**Project** · Software · Open

### 24 Sep 2026 · [#17 Problem-mining and opportunity discovery system](https://github.com/afnannuzulasugihartono/brain-dump/issues/17)

Idea generation is often biased toward interesting solutions rather than recurring problems with real economic value. A disciplined problem-mining pipeline can improve the quality of what gets explored.

**Exploring** · Research · Open

### 24 Sep 2026 · [#16 Cross-tool memory wallet for AI context](https://github.com/afnannuzulasugihartono/brain-dump/issues/16)

Useful context is currently fragmented across assistants and tools. A portable memory layer could reduce repeated setup and let users carry durable context between AI products.

**Exploring** · Software · Open

### 24 Sep 2026 · [#15 BarangPas as an independent product curator](https://github.com/afnannuzulasugihartono/brain-dump/issues/15)

Consumers often face noisy affiliate recommendations and unclear incentives. A consistent research and curation standard could create more trust and a stronger long-term brand.

**Exploring** · Business · Open

### 24 Sep 2026 · [#14 XTRA HANTU — Paste 300 Link Detector](https://github.com/afnannuzulasugihartono/brain-dump/issues/14)

Affiliate links can silently lose commission eligibility while still appearing valid, and checking large link sets manually is slow and easy to miss. The intended “magic moment” is paste → useful result in under 30 seconds.

**Exploring** · Product · Open

### 24 Sep 2026 · [#13 On-demand shift-work matching](https://github.com/afnannuzulasugihartono/brain-dump/issues/13)

Short-term labor demand and worker availability are often highly time-sensitive, but existing hiring workflows are too slow for immediate operational needs.

**Exploring** · Product · Open

### 24 Sep 2026 · [#12 Productive Opportunity Network](https://github.com/afnannuzulasugihartono/brain-dump/issues/12)

Many productive opportunities fail to connect because demand, available capacity, location, requirements, constraints, and economic value are fragmented across different channels.

**Exploring** · Product · Open

### 24 Sep 2026 · [#11 Per-idea history and lifecycle](https://github.com/afnannuzulasugihartono/brain-dump/issues/11)

History makes Brain Dump useful as a memory of how ideas developed, not just a list of their current state. It should live inside each idea rather than becoming another sidebar destination.

**Exploring** · Software · Open

[View all 11 ideas →](TIMELINE.md)
<!-- TIMELINE:END -->

## Workflow

Inbox → Exploring → Promising → Project → Archived

GitHub Issues are the source of truth. Review signals are derived from Issues; optional AI insight is non-canonical. See [AGENTS.md](AGENTS.md) for the AI write policy.
